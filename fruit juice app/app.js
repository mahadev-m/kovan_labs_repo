...
list.addEventListener("click", (e) => {
  ...
  if (element.classList[0] === "button") {
    ...
    total =
      total - parseFloat(elementDataPrice) * parseInt(elementDataQuantity);
    elementNode.remove();
  ...
  fruitList.forEach((fruit) => {
    itemHtml = `
            <div class="mt-sm-3 bg-light alert flexview">
                ...
                <button type="button" class="button col-sm-2 remove-btn" data-name="${fruit}" data-price="${
      state[`${fruit}`].price
    }" data-quantity="${state[`${fruit}`].quantity}">Remove</button>
            </div>`;
